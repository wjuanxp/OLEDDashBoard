...
| Widgets + engine state (vtable-free) | ≈ 130 B |
...
- **New widgets**: derive from `Widget`, implement `update()`/`draw()`, then
  add the instance to `OLEDDashboard` (and wire it into `update()`/`draw()`,
  which call widgets directly to keep the 2 KB-SRAM footprint vtable-free).
